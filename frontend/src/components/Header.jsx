...
const Header = () => {
  ...
  },[menuOpened]) /*dependency array when menuopen */
  return (
    <header className='max-padd-container fixed top-1 w-full left-0 right-0 z-50'>
      {/* {container } */}
      <div className={`${active ? "py-0" : "py-1"} max-padd-container bg-white transition-all duration-200 rounded-full px-5
      ring-1 ring-slate-900/5`}>
        <div className='flexBetween py-3'>
          ...
          <div className='flexCenter gap-x-4'>
            ...
            {/* Mobile  */}
            <Navbar containerStyles={`${menuOpened ? "flex items-start flex-col gap-y-8 capitalize fixed top-20 right-8 p-12 bg-white rounded-3xl shadow-md w-64 medium-16 ring-slate-900/5 transition-all duration-300 z-50" 
          ...
          <div className='flexBetween gap-x-3 sm:gap-x-5 bold-16'>
            {!menuOpened ? (<MdMenu onClick={toggleMenu} className='xl:hiddden cursor-pointer text-3xl hover:text-secondary'/>) :
             (<MdClose onClick={toggleMenu} className='xl:hiddden cursor-pointer text-3xl hover:text-secondary' />)}
