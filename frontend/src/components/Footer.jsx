  ...
  return (
    <footer className='max-padd-container mb-4'>
      <div className='max-padd-container bg-primary rounded-tr-3xl pt-12 xl:pt-20 pb-8'>
        ...
        <p>Beneath the whispering trees, shadows danced like secrets waiting to be uncovered. A distant melody drifted through the air.</p>
          ...
          <div className='max-w-sm'>
            <Link to={'/'} className='flex items-center gap-x-2'>
              <span className='font-[900] text-[24px]'>
                Land<span className='font-[600] medium-20'>X</span></span>
            </Link>
            <p className='py-4'>Beneath the whispering trees, shadows danced like secrets waiting to be uncovered. A distant melody drifted through the air, painting the twilight with hints of forgotten dreams.</p>
              ...
              <FooterColumn title={SOCIALS.title}>
                <ul className='flex-gap-4'>
                  {SOCIALS.links.map((link) => (
                      <Link to={'/'} key={link.id} className='text-3xl'>{link.icon}</Link>
                  ))}
                </ul>
              </FooterColumn>
            </div> */}
      ...
      <p className='text-white bg-tertiary medium-14 py-2 px-8 rounded-b-3xl flexBetween'>
        <span>2025 LandX</span> All rights reserved</p>
    </footer>
  );
}
